...
internal object TypeResolver {
	...
	fun resolveType(schema: GSchema, ref: GTypeRef, includeIntrospection: Boolean = true): GType? = when (ref) {
	...
	fun resolveType(schema: GSchema, ref: GNamedTypeRef, includeIntrospection: Boolean = true): GNamedType? = resolveType(schema, ref.name, includeIntrospection)

	fun resolveType(schema: GSchema, name: String, includeIntrospection: Boolean = true): GNamedType? = schema.resolveType(name) ?: when (includeIntrospection) {
