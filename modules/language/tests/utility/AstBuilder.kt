...
import io.fluidsonic.graphql.GDirectiveArgumentDefinition
...
fun <T : GNode> ast(configure: AstBuilder.() -> T) = AstBuilder.run(configure)

fun documentAst(origin: IntRange, configure: AstBuilder.DocumentBuilder.() -> Unit) = AstBuilder.DocumentBuilder(origin).apply(configure).build()

@AstBuilderDsl
object AstBuilder {

	fun document(origin: IntRange, configure: DocumentBuilder.() -> Unit) = DocumentBuilder(origin).apply(configure).build()
...
// 		}
	...
	data class DocumentPosition(override val startPosition: Int, override val endPosition: Int, override val line: Int = -1, override val column: Int = -1) :
		GDocumentPosition {
		...
		override fun toString() = "$startPosition .. $endPosition"
	...
	class TypeReferenceBuilder {
		...
		fun build() = type ?: error("name(), list() or nonNull() missing")
	...
	class ValueBuilder {
		...
		fun build() = value ?: error("boolean(), or …FIXME missing")
