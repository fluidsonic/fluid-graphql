...
public class GPath(elements: List<Element> = emptyList()) {
	...
	public fun addIndex(index: Int): GPath = GPath(elements + Element.Index(index))

	/** Returns a new path with [name] appended as a field-name segment. */
	public fun addName(name: String): GPath = GPath(elements + Element.Name(name))

	override fun equals(other: Any?): Boolean = (this === other || (other is GPath && elements == other.elements))
	...
	public companion object {
		...
		public fun ofName(name: String): GPath = GPath(listOf(Element.Name(name)))
	}
	...
	public class Builder {
		...
		public fun snapshot(): GPath = GPath(
			stack.map { element ->
				when (element) {
	...
	public sealed class Element {
		...
		public class Name(public val value: String) : Element() {
			...
			override fun equals(other: Any?): Boolean = (this === other || (other is Name && value == other.value))
		...
		public class Index(public val value: Int) : Element() {
			...
			override fun equals(other: Any?): Boolean = (this === other || (other is Index && value == other.value))
...
internal inline fun <Result> GPath.Builder?.withName(name: String, block: () -> Result): Result = if (this !== null) {
	withName(name, block)
} else {
	block()
}

internal inline fun <Result> GPath.Builder?.withIndex(index: Int, block: () -> Result): Result = if (this !== null) {
	withIndex(index, block)
} else {
	block()
}
