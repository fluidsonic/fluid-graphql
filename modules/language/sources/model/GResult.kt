 ...
 */
public sealed class GResult<out Value>(public val errors: List<GError>) {
	...
	public companion object {
		 ...
		 */
		public inline fun <Value> catchErrors(action: () -> Value): GResult<Value> = try {
			success(action())
		} catch (exception: GErrorException) {
			failure(exception.errors)
		}

		public fun failure(errors: List<GError>): GResult<Nothing> = Failure(errors)

		public fun failure(error: GError): GResult<Nothing> = Failure(error)

		public fun success(): GResult<Nothing?> = success(value = null)

		@Suppress("UNCHECKED_CAST")
		public fun <Value> success(value: Value, errors: List<GError> = emptyList()): GResult<Value> = when {
	...
	@PublishedApi
	internal class Failure(errors: List<GError>) : GResult<Nothing>(errors = errors) {
		...
		override fun equals(other: Any?) = this === other || (other is Failure && errors == other.errors)

		override fun hashCode() = errors.hashCode()

		override fun toString() = "Failure($errors)"
	}

	@PublishedApi
	internal class Success<Value>(val value: Value, errors: List<GError> = emptyList()) : GResult<Value>(errors = errors) {

		init {
			require(value !is GResult<*>) { "A GResult cannot be the value of another GResult." }
		}

		override fun equals(other: Any?) = this === other || (other is Success<*> && value == other.value && errors == other.errors)

		override fun hashCode() = value.hashCode() xor errors.hashCode()
 ...
 * ```
 */
public inline fun <Value> GResult<Value>.ifErrors(action: (result: List<GError>) -> Nothing): Value = when {
...
public inline fun <Value> GResult<Value>.flatMapErrors(action: (errors: List<GError>) -> GResult<Value>): GResult<Value> = when (this) {
...
public inline fun <Value> GResult<Value>.mapErrors(action: (errors: List<GError>) -> List<GError>): GResult<Value> = when (this) {
 ...
 */
public inline fun <Value, TransformedValue> GResult<Value>.mapValue(action: (value: Value) -> TransformedValue): GResult<TransformedValue> = when (this) {
