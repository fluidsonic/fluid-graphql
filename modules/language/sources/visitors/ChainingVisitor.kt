...
private class ChainingVisitor<out Result, IntermediateResult, in Data>(
	...
	override fun onNode(node: GNode, data: Data, visit: Visit) = nextCoordinator.visit(node = node, data = current.onNode(node, data = data, visit = visit))
		.also { visit.abort() } // we're only interested in the root node
}

@InternalGraphqlApi
public fun <Result, IntermediateResult, Data> Visitor<IntermediateResult, Data>.then(next: Visitor<Result, IntermediateResult>): Visitor<Result, Data> =
...
): Visitor<Result, Data> = ChainingVisitor(current = this, nextCoordinator = nextCoordinator)
