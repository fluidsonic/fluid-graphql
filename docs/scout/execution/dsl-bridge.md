...
Scalars have three separate coercer channels, not one: node input (inline document values, `coerceNodeInput`), variable input (`coerceVariableInput`), and output (`coerceOutput`). Registering only one channel leaves the others on built-in behavior — a frequent source of asymmetry (see variable-coercion.md for which channel handles default values).

Related: resolver-wiring.md, coercer-chaining.md.
