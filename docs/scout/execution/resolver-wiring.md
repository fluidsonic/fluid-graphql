...
**The executor-level `fieldResolver` runs FIRST, not as fallback.** `GExecutor.default`'s KDoc calls it a fallback, but `DefaultFieldSelectionExecutor.resolveFieldValue` invokes it for every field when non-null; the definition-attached resolver only runs if it calls `context.next()`. `DefaultFieldResolverContext.next()` throws `IllegalStateException` when no further resolver exists — a wiring bug that deliberately escapes `execute()` rather than becoming a GraphQL error, and there is no property-lookup default resolver to fall back on. Its `resolverForField` has exactly one built-in fallback: a field of an introspection type is resolved from the `IntrospectionResolvers` table (introspection.md), which is why `GFieldDefinition.resolver` answers `null` for `__Type.fields` and friends.
