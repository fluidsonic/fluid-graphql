...
`GType.isSupertypeOf`/`isSubtypeOf` mix two mechanisms: direct type comparison is instance identity (`===`) for most kinds, while interface implementation and union membership are checked by name against `GNamedTypeRef` lists (`GInterfaceType.isSupertypeOf`, `GUnionType.isSupertypeOf`). So identity-based checks only work between `GType` instances resolved from the same `GSchema` — object, enum and input-object types from two separately parsed schemas are unrelated (`isSubtypeOf` returns false negatives) — while name-based interface/union membership succeeds even across schemas.

Scalars are the one exception, and no longer via singletons: `GScalarType.equals` compares by **name and class**, so any two schemas' `Int` types match and a `GCustomScalarType` named `Int` does not. See builtin-scalar-instances.md.
