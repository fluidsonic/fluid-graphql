...
**Children are visited automatically** when a handler returns without calling `visitChildren()` or `skipChildren()`. Corollary: calling `visit.visitChildren()` mid-handler and then running more code is the post-order idiom — the trailing code runs after the whole subtree. Validation rules like `AllVariablesUsedRule` and `FragmentDefinitionUsageRule` depend on this; their `visitChildren()` calls look redundant but removing them makes post-checks run against empty collections. A second `visitChildren()` on the same node throws; after `skipChildren()` it is a silent no-op.
